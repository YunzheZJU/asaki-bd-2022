...
s`.s3 .bg .line-box .near div`.size(8.5, 40).css({ 'border-radius': '999px' }).translate(-15, 0)
                              .animate('translate_y_from_80rem_to_-80rem', { begin: a.s3_bg_appear.begin + 0.1, dur: 1.2, fill: 'both', iterations: 2 })
s`.s3 .bg .line-box .middle div`.css({ 'border-radius': '999px', filter: 'blur(0.5rem)' })
...
s`.s3 .bg .line-box .far div`.size(8.5, 140).css({ 'border-radius': '999px', filter: 'blur(3rem)' })
...
s`.s3 .frame-top`.align(LEFT, TOP).size(FULL, s3_frame_height).fill(BLACK).animate('opacity_from_0_to_1', { begin: s3_begin, dur: 0, fill: 'both' })
s`.s3 .frame-bottom`.align(RIGHT, BOTTOM).size(FULL, s3_frame_height).fill(BLACK).animate('opacity_from_0_to_1', { begin: s3_begin, dur: 0, fill: 'both' })
...
s`.s3 .text-box`.css({ padding: '2rem 3rem', color: WHITE }).fill('#2f2c16').align_children(CENTER, BOTTOM)
                .animate('opacity_from_0_to_1', { begin: s3_begin + 0.1, dur: 0.1, fill: 'both' }, 's3_text_box_appear')
                ...
                .animate('opacity_from_1_to_0', { begin: a.s3_text_box_move.end - 0.6, dur: 0.6, fill: 'forwards' }, 's3_text_box_disappear')
...
//                      .animate({ translate: [[0, 0], [-5, 3]] }, { begin: a.s3_text_box_disappear.begin, dur: a.s3_text_box_disappear.dur, fill: 'forwards', easing: s3_text_disappear_easing })
s`.s3 .bg .lazer-box`.align(LEFT, TOP).size(FULL, FULL).align_children(CENTER, CENTER)
                     .animate('opacity_from_0_to_1', { begin: a.s3_text_box_move.begin + a.s3_text_box_move.dur * 0.8, dur: 0.2, fill: 'both' }, 's3_bg_lazer_box_appear')
s`.s3 .bg .lazer-box div`.align(LEFT, TOP).size(FULL, FULL).css({ 'transform-style': 'preserve-3d', perspective: '4rem' })
                         .forEach((ele, index) => {
                           ele.rotate(36 * index + Math.random() * 20 - 10)
                              .before.align({ [LEFT]: '50p', [TOP]: '50p' }).origin(CENTER, TOP).translate('-50p', '20p', -4).rotate_x(5).fill('currentColor').css({ 'border-radius': '999px' })
                              .size(8.5, 60 + 100 * Math.random()).animate('translate_z_from_-185rem_to_5rem', { begin: a.s3_bg_lazer_box_appear.begin + 0.1 * Math.random(), dur: 0.6 + 0.4 * Math.random(), fill: 'both', easing: '>', iterations: 1})
                         })
...
s`.s3 .bg .line-box`.animate('opacity_from_1_to_0', { begin: a.s3_frame_disappear.begin, dur: 0, fill: 'both' })
